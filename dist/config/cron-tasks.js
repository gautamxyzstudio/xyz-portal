    ...
    createDailyAttendance: {
        task: async ({ strapi }) => {
            try {
                ...
                if (result.success) {
                    strapi.log.info(`✅ Daily attendance entries created for ${result.totalUsers} users`);
                    strapi.log.info(`📊 Created: ${result.createdEntries.length}, Errors: ${result.errors.length}`);
                }
                else {
                ...
                strapi.log.error('❌ Error in createDailyAttendanceEntries cron:', error);
        ...
        task: async ({ strapi }) => {
            try {
                ...
                if (result.success) {
                    ...
                    strapi.log.info(`📈 Present: ${result.stats.present}, Absent: ${result.stats.absent}, Late: ${result.stats.late}`);
                }
                else {
                    strapi.log.error('❌ Failed to generate weekly report:', result.error);
                }
                return result;
            }
            catch (error) {
