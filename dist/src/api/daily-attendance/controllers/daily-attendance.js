...
exports.default = {
    ...
    async checkIn(ctx) {
        ...
        const userDetails = await strapi.entityService.findOne('plugin::users-permissions.user', user);
        if (userDetails.user_type === 'Admin' || userDetails.user_type === 'Hr') {
            return ctx.badRequest('Attendance tracking is not required for Admin and Hr users');
        }
    ...
    async debugTable(ctx) {
        try {
            ...
            const records = await strapi.entityService.findMany('api::daily-attendance.daily-attendance', {
                limit: 1,
            });
            ctx.body = {
                success: true,
