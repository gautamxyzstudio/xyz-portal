...
module.exports = createCoreController(moduleUid, ({ strapi }) => ({
    async create(ctx) {
        ...
        try {
            ...
            const leave = await strapi.entityService.create("api::leave-status.leave-status", {
                data,
                populate: ["user"],
            });
          ...
          `,
                    ...
                    console.error(`❌ Failed to send email to HR (${hr.email}):`, err.message);
                }
            }
    ...
    async approve(ctx) {
        try {
            ...
            const leaveDays = calculateLeaveDays(leaveRequest.start_date, leaveRequest.end_date, leaveDuration, leaveRequest.is_first_half);
            // ------------------------------------------------------------------
            // Update user leave balances
            // ------------------------------------------------------------------
            const userData = await strapi.entityService.findOne("plugin::users-permissions.user", user.id, { fields: ["leave_balance", "unpaid_leave_balance"] });
